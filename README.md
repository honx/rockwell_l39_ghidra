...
│   │   └── L3902PropagateConfigs.java
│   │                               Walks the CFG to tag each instruction
│   │                               with its active configuration, then
│   │                               rewrites $0200-$7FFF references to
│   │                               the right cfgN overlay block.
...
- **Tests**: 95 assertions across 4 test scripts, all green against
...
- Bank-aware cross-references: Phases 1-4 all done. The four scripts
  produce per-configuration overlays, identify the four
  `switch_rom_cfgX()` functions, redirect 149 ROM6 references, and
  propagate configuration through the CFG to add 2125 cfgN-resolved
  references. The simplified propagator assumes "callees return with
  the caller's config unless they are directly a switch_rom function";
  inter-procedural fixpoint analysis would be a tighter follow-up but
  the simpler model is correct on this firmware. See
  `docs/open-points.md` for the remaining caveats.
